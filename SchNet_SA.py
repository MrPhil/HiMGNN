...
class update_e(torch.nn.Module):
    ...
    def forward(self, v, dist, dist_emb, edge_index):
        ...
        W = self.mlp(dist_emb.to('cuda:0')) * C.view(-1, 1).to('cuda:0')
        v = self.lin(v)
        e = v[j] * W
        return e
...
class update_v(torch.nn.Module):
    ...
    def forward(self, v, e, edge_index):
        _, i = edge_index
        out = scatter(e, i, dim=0).to('cuda:0')
        out = self.lin1(out)
        out = self.act(out)
        out = self.lin2(out)
        return v + out
...
class emb(torch.nn.Module):
    ...
    def forward(self, dist):
        dist=dist.to('cpu')
        self.offset=self.offset.to('cpu')
...
class SA(nn.Module):
    ...
    def forward(self, z, batch):
        ew = torch.exp(self.project(z))
        return scatter((ew/(torch.index_select(scatter(ew, batch, dim=0), 0, batch)) * z), batch, dim=0)


class SchNet_atten(torch.nn.Module):
    r"""
    ...
    def __init__(self, energy_and_force=False, cutoff=10.0, num_layers=3, hidden_channels=16, num_filters=16,
                 num_gaussians=16,out_channels = 1):
        super(SchNet_atten, self).__init__()
        ...
        self.init_v = Embedding(32, hidden_channels)
    ...
    def forward(self, batch_data):
        ...
        edge_index = radius_graph(pos, r=self.cutoff, batch=batch).to('cpu')
        row, col = edge_index
        dist = (pos[row] - pos[col]).norm(dim=-1).to('cpu')
        ...
        for update_e, update_v in zip(self.update_es, self.update_vs):
            e = update_e(v, dist, dist_emb, edge_index).to('cpu')
